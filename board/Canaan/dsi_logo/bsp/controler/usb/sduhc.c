...
static void sduhcUpdateTimeInterruptTransfer(ListHead * head, UhcHostEpPriv * lastFinished) {
    ...
    for (listEntry = head->next; listEntry != head; listEntry =
        ...
        printf("===usbHEpPriv->frame = 0x%x time=%d\n", usbHEpPriv->frame, time);
...
static UhcHostEpPriv * sduhcFindFirstInterruptTransfer(ListHead * head) {
    ...
    if (usbHEpPrivActual) {
        vDbgMsg(DBG_CUSBH_VERBOSE_MSG, 0,
...
static void sduhcEpProgram(UHC_Controller * pD, UhcEp * hwEp,
    ...
    if (!hwEp->isInEp) {
        ...
        if (hwEp->hwEpNum) {
            ...
            // printf("before:hwEpNum=%d, frame=%x\n",hwEp->hwEpNum, usbEpPriv->frame);
            // usbEpPriv->frame = 1<<15;
            // usbEpPriv->frame |= 0x81<<24;
            CPS_UncachedWrite32(&regs->txsoftimer[hwEp->hwEpNum], 0x81001000/*usbEpPriv->frame*/);
            // printf("after:hwEpNum=%d, frame=%x 0x%x\n",hwEp->hwEpNum, usbEpPriv->frame, CPS_UncachedRead32(&regs->txsoftimer[hwEp->hwEpNum]));
        ...
        if (hwEp->hwEpNum) {
            ...
            // usbEpPriv->frame = 1<<15;
            // usbEpPriv->frame |= 0x81<<24;
            CPS_UncachedWrite32(&regs->rxsoftimer[hwEp->hwEpNum], 0x81001000/*usbEpPriv->frame*/);
            // printf("after:hwEpNum=%d, frame=%x 0x%x\n",hwEp->hwEpNum, usbEpPriv->frame, CPS_UncachedRead32(&regs->rxsoftimer[hwEp->hwEpNum]));
...
static uint32_t sduhcInit(void *privateData, const CUSBH_Config* config,
    ...
    if(speed_select)
    {
        CPS_UncachedWrite8(&pD->regs->speedctrl, 0x00);
    }
    else
    {
...
static void sduhcIsr(void *privateData) {
    ...
    if(pD->uhcHostDrv->isHostMode(pD)) {
        UCRegs *usb_regs = pD->regs;
        uint8_t extirq = CPS_UncachedRead8(&usb_regs->reserved4);
        // printf("extirq=0x%x\n", extirq);
        if(extirq & 0x80)//Wake up interrupt request
        {
            CPS_UncachedWrite8(&usb_regs->reserved4, extirq);
            CPS_UncachedWrite8(&usb_regs->reserved6, 0x00);
            ...
            uint8_t otgctrl = CPS_UncachedRead8(usbRegs->otgctrl);
            CPS_UncachedWrite8(&usbRegs->otgctrl, otgctrl | 0x1);//write the busreq bit to initialize the resume signalling. 
            CPS_UncachedWrite8(&usb_regs->reserved6, 0xff);
        }
    }
...
static uint32_t sduhcReqQueue(void *pD, CUSBH_Req * usbReq) {
    ...
    // printf("====usbHEpPriv->interval = 0x%x\n", usbHEpPriv->interval);
...
static void sduhc_print_reg(void* pD)
{
    ...
    // uint8_t otg_ctrl = ctrl->regs->otgctrl;
    // printf("otgctrl=0x%x \n", otg_ctrl);
    // // CPS_UncachedWrite8(&ctrl->regs->portctrl, 0x20 );
    // CPS_UncachedWrite8(&ctrl->regs->otgctrl, otg_ctrl&(~0x1) );
    // printf("otgctrl=0x%x \n", ctrl->regs->otgctrl);
    
    // uint8_t usbcs = ctrl->regs->usbcs;
    // printf("pre usbcs=0x%x \n", usbcs);
    // CPS_UncachedWrite8(&ctrl->regs->usbcs, usbcs|(0x20) );
    // printf("after usbcs=0x%x \n", ctrl->regs->usbcs);

    // uint8_t clkgate = ctrl->regs->clkgate;
    // printf("pre clkgate=0x%x \n", clkgate);
    // CPS_UncachedWrite8(&ctrl->regs->clkgate, 0x1 );
    // printf("after clkgate=0x%x \n", ctrl->regs->clkgate);
...
/*
    USB Suspend in OTG state (A_HOST -> A_SUSPEND)
    When the OTG FSM is in the a_host state, the microprocessor can suspend the USB bus. 
    To force a suspend state, the microprocessor clears the busreq (otgctrl[0]) bit. 
    After the busreq bit is cleared the OTG FSM enters the a_suspend state and the HC stops any bus activity 
    (data transfers and SOF generation).
    When the OTG FSM is in the a_suspend state less than 3125 us and the microprocessor sets the busreq bit, 
    the OTG FSM immediately returns to the a_host state. 
    When the OTG FSM is in the a_suspend state for more than 3125 us and the microprocessor writes the busreq bit, 
    the cores initializes resume signalling.
    When the OTG FSM enters the a_suspend state, the microprocessor can switch off the UTMI clock 
    by writting the clkgate register. 
    After entering a_suspend the microprocessor should wait for at least 3125 us and then 
    it can write the clkgate register and switch off the UTMI clock.
    The CUSB2 generates the suspend interrupt request (usbirq[3] - suspir bit) after 3125 us.
*/
static void sduhc_suspend(void* pD, uint8_t methord) {
    UHC_Controller * ctrl = (UHC_Controller*)pD;

    printf("==========sduhc_suspend=========\n");
    uint8_t *preg = (uint8_t *)ctrl->regs;
    printf("pre otgstate=0x%x methord=%d\n", ctrl->regs->otgstate, methord);
    ...
    else if(methord==2)
    {
        ...
        CPS_UncachedWrite8(&ctrl->regs->otgctrl, otg_ctrl&(~0x1) );
        printf("otgctrl=0x%x \n", ctrl->regs->otgctrl);
        while(ctrl->regs->otgstate != CUSBH_OTG_STATE_A_SUSPEND);
        msleep(4);
        CPS_UncachedWrite8(&ctrl->regs->clkgate, 0xff );
    }
    printf("after otgstate=0x%x \n", ctrl->regs->otgstate);
}

static void sduhc_resume(void* pD, uint8_t methord) {
    UHC_Controller * ctrl = (UHC_Controller*)pD;
...
static void sduhc_reset(void* pD, uint8_t methord) {
    UHC_Controller * ctrl = (UHC_Controller*)pD;
    ...
    uint8_t portctrl = ctrl->regs->portctrl;
    CPS_UncachedWrite8(&ctrl->regs->portctrl, portctrl | 0x20 );
